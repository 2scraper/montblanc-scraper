#!/usr/bin/env python3
"""
montblanc-scraper — 2captcha Scraper API edition (fourth engine)
================================================================

Fetches a Montblanc page through 2captcha's Scraper API — a hosted browser
that returns rendered HTML over plain HTTPS — and parses it with the same
`product_parser` the three local engines use. No browser to install, no
driver, no Chromium download.

When this path is the right one, and when it is not
----------------------------------------------------
Say it plainly rather than selling it (§13). **Montblanc served an ordinary
datacenter address on every mode measured** (2026-09-17, from a Hetzner IP:
listings, search and product pages all HTTP 200, no key, no proxy, no
challenge). So this client is not a way IN — nothing is keeping you out.
...
What it cannot do, and this one matters on this site:
  ...
  * **It cannot choose a MARKET by exit.** Montblanc sets prices per market
    and the market is in the URL path, so ask for the locale you want
    directly — `/de-de/…` rather than a German exit. That works from any
    address, which is the useful half of this site being ungated.

One thing to know before scripting against the site yourself
-------------------------------------------------------------
Montblanc's edge refuses a request whose User-Agent names an HTTP client
library, and it refuses it by killing the connection rather than answering —
`requests` sees a ReadTimeout, which looks exactly like a slow network. This
client goes through the Scraper API's own browser, so it never meets that;
see page_flow.classify_transport_error if you hit it elsewhere.
...
-----
    ...
    # $MONTBLANC_CDP_ENDPOINT, so neither needs to be typed — a secret in
...
def _run_once(args, attempt: int = 1, attempts: int = 1) -> int:
    ...
    if state == "blocked":
        ...
        with open(dump, "w", encoding="utf-8") as f:
            f.write(html)
        logger.error(
            "Montblanc did not serve the Scraper API's request (upstream "
    ...
    if vendor:
        ...
        logger.error("A challenge page is not a final answer — retry before "
                     "concluding anything (--retries). On Montblanc no challenge "
    ...
    if args.mode == "product":
        # One row per VARIANT, out of the page's ProductGroup — the same
        # shape the local engines produce, so downstream code does not
        # branch on which engine fetched.
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Montblanc scraper — 2captcha Scraper API edition (no "
                    "local browser). Useful when you cannot install "
                    "Chromium; not a way past anything, because Montblanc "
    ...
    p.add_argument("--url", default=None,
                   help="A montblanc.com URL — a category listing, a search, "
                        "or one product page with --mode product. Required, "
                        "unless MONTBLANC_URL is set in the environment or "
                        "in .env.")
    ...
    p.add_argument("--out", default="montblanc_products_scraperapi", help="Output file prefix")
    ...
    env_config.apply(args, keys={
        "TWOCAPTCHA_KEY": "key",
        "MONTBLANC_CDP_ENDPOINT": "cdp_url",
        "MONTBLANC_URL": "url",
    })
    if not args.url:
        p.error("no --url given, and MONTBLANC_URL is not set in the environment "
                "or in .env.")
    return args
